...
function fetchApi(params){
	...
	var  url = 'http://dev.jiuziran.com/api';
    //var url = 'http://127.0.0.1:8080/adminsystem/api'
    if(getRequestParameter('origin')=='ios'){
		url = 'https://dev.jiuziran.com/api';
    }
...
function getRootPath(){
    return 'http://dev.jiuziran.com/htl2/v1/'
    //return 'file:///E:/Project/adminsystem/frw/htl2/v1/'
}

//获取微信js权限路径
function getWxConfig(url){
    var params = {"url":url};
    return fetchUrlApi("https://wxshop.jiuziran.com/getWxConfig.html",params)
       .then(res=>res)
}

//搜索店铺内商品列表
function searchByShop(dsp_id,key){
    var params = {"params":"{'biz':'com.action.api.v2.search.shopitem','data':{'dsp_id':'"+dsp_id+"','key':'"+key+"'}}"}
    return fetchApi(params)
        .then(res=>res)
}

//读取消息
function readPush(push_id){
    var params = {"params":"{'biz':'com.action.api.push.read','data':{'push_id':'"+push_id+"'}}"}
    return fetchApi(params)
        .then(res=>res)
}
