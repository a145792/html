...
var vm = new Vue({
    ...
    methods:{
        search:function(){
            if(vm.origin == 'adr'){
                APP.appSearch(vm.shop_id)
            }else if(vm.origin == 'ios'){
                window.webkit.messageHandlers.appSearch.postMessage(vm.shop_id)
            }else if(vm.origin == 'wxshop'){
                window.location.href = getRootPath() + 'search.html?shop_id=' + vm.shop_id + '&ccr_id=' + vm.ccr_id
            }
        }
    }
...
//--------------------------------------------------页面的事件----------------------------------------------------------------
    ...
    //点击购物车
    $(document).on('click','.shopcar_div',function(){
        window.location.href = wxpath + '/shopcar.html';
    })
...
function removeStyle(){
    ...
    $('.topImg').css('top','10px');
    $('.topImg-sousuo').css('top','10px')
    $('.topImg-sousuo').css('position','absolute');
